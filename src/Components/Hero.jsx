  ...
  return (
    ...
    <div className="relative flex items-center justify-center">
        ...
        <div className="absolute inset-0 flex items-center justify-center h-100">
            {/* Larger Circle */}
            <motion.div initial={{opacity: 0 }} animate={{opacity:1 }} transition={{duration: 0.5, delay: 0.7}} className="relative w-[900px] h-[900px] overflow-hidden lg:overflow-visible  ">
      ...
      <div className="relative z-10 text-center max-w-6xl px-4">
        ...
        <motion.h1 initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity:1 }} transition={{duration: 0.5, delay: 0.9}} className="text-4xl md:text-6xl font-bold mb-6 text-gray-200">      
        ...
        <motion.p initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity:1 }} transition={{duration: 0.5, delay: 1}} className="text-lg text-gray-200 mb-8 max-w-3xl mx-auto font-light">
          Our advanced AI analyzes your papers, extracting key insights and generating 
          comprehensive summaries in seconds
        </motion.p>

        {/* CTA Button */}
        <motion.div initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity:1 }} transition={{duration: 0.5, delay: 1}} className="flex justify-center gap-4">
        ...
        {/* Stats */}
        <div initial={{ y: 100, opacity: 0 }}  whileInView={{ y: 0, opacity:1 }} transition={{duration: 0.5, delay: 1}} className="my-32 grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-14 max-w-4xl mx-auto">
          {[
            { label: 'Papers Analyzed', value: '100K+' },
            { label: 'Processing Time', value: '< 60s' },
            { label: 'Accuracy Rate', value: '98%' }
          ].map((stat, index) => (
            <motion.div initial={{ y: 100, opacity: 0 }}  whileInView={{ y: 0, opacity:1 }} transition={{duration: 0.5, delay: 0.4}} key={index} className="relative bg-gray-900/50 backdrop-blur-md border border-gray-700 rounded-xl py-6 px-10">
